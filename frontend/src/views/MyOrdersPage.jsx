import { useEffect, useState } from 'react';
import { useNavigate } from 'react-router-dom';
import KhaledMode from '../assets/KhaledMode.avif'
import OspanAli from '../assets/OspanAli.avif'
import SleeveLessWear from '../assets/SleeveLessWear.avif'
import SingleWear from '../assets/SingleWear.avif'

const MyOrdersPage = () => {
	const [orders, setOrders] = useState([]);
	const navigate = useNavigate();
	useEffect(() => {
		// Simulate order fetch
		setTimeout(() => {
			const mockOrders = [
				{
				  _id: '12345',
				  createdAt: new Date('2024-12-01T10:00:00Z'),
				  shippingAddress: { city: 'New York', country: 'USA' },
				  orderItems: [{ name: 'Monochrome Majesty', image: [{ url: KhaledMode }] }],
				  totalPrice: 130,
				  isPaid: false,
				},
				{
				  _id: '23456',
				  createdAt: new Date('2025-01-15T14:30:00Z'),
				  shippingAddress: { city: 'Los Angeles', country: 'USA' },
				  orderItems: [{ name: 'Cultural Blend', image: [{ url: OspanAli }] }],
				  totalPrice: 100,
				  isPaid: true,
				},
				{
				  _id: '34567',
				  createdAt: new Date('2025-02-20T09:45:00Z'),
				  shippingAddress: { city: 'Chicago', country: 'USA' },
				  orderItems: [{ name: 'Bold & Breezy', image: [{ url: SleeveLessWear }] }],
				  totalPrice: 60,
				  isPaid: true,
				},
				{
				  _id: '45678',
				  createdAt: new Date('2025-03-10T16:15:00Z'),
				  shippingAddress: { city: 'San Francisco', country: 'USA' },
				  orderItems: [{ name: 'Minimal Moves', image: [{ url: SingleWear }] }],
				  totalPrice: 70,
				  isPaid: true,
				},
			  ];			  
			setOrders(mockOrders);
		});
	}, []);
	const handleRowClick = (orderId) => {
		navigate(`/order/${orderId}`);
	};
	return (
		<div className='max-w-7xl mx-auto p-4 sm:p-6'>
			<h2 className='text-xl sm:text-2xl font-bold mb-6'>My Orders</h2>
			<div className='relative shadow-md rounded-md sm:rounded-lg overflow-hidden'>
				<table className='min-w-full text-left text-gray-500'>
					<thead className='bg-gray-100 text-xs uppercase text-gray-700'>
						<tr>
							<th className='py-2 px-4 sm:py-3'>Image</th>
							<th className='py-2 px-4 sm:py-3'>Order Id</th>
							<th className='py-2 px-4 sm:py-3'>Created</th>
							<th className='py-2 px-4 sm:py-3'>Shipping Adress</th>
							<th className='py-2 px-4 sm:py-3'>Items</th>
							<th className='py-2 px-4 sm:py-3'>Price</th>
							<th className='py-2 px-4 sm:py-3'>Status</th>
						</tr>
					</thead>
					<tbody>
						{orders.length > 0 ? (
							orders.map((order) => (
								<tr
									onClick={() => handleRowClick(order._id)}
									key={order._id}
									className='border-b hover:border-gray-50 cursor-pointer'>
									<td className='p-2 sm:p-4'>
										<img
											className='w-10 h-10 sm:w-12 sm:h-12 object-cover rounded-lg'
											src={order.orderItems[0].image[0].url}
											alt={order.orderItems[0].name}
										/>
									</td>
									<td className='p-2 sm:p-4 font-medium text-gray-900 whitespace-nowrap'>#{order._id}</td>
									<td className='p-2 sm:p-4'>
										{new Date(order.createdAt).toLocaleDateString()} {new Date(order.createdAt).toLocaleTimeString()}
									</td>
									<td className='p-2 sm:p-4'>
										{order.shippingAddress ? `${order.shippingAddress.city}, ${order.shippingAddress.country}` : 'N/A'}
									</td>
									<td className='p-2 sm:p-4'>{order.orderItems.length}</td>
									<td className='p-2 sm:p-4'>${order.totalPrice}</td>
									<td className='p-2 sm:p-4'>
										<span
											className={`${
												order.isPaid ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'
											} p-2 rounded-full font-medium`}>
											{order.isPaid ? 'Paid' : 'Pending'}
										</span>
									</td>
								</tr>
							))
						) : (
							<tr>
								<td colSpan={7} className='py-4 px-4 text-center text-gray-500'>
									You have no orders
								</td>
							</tr>
						)}
					</tbody>
				</table>
			</div>
		</div>
	);
};
export default MyOrdersPage;
